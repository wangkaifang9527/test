...
Page({
  ...
  onLoad: function (options) {
    ...
    url.ajaxPost(false, url.focusList, {
  ...
  callbackData: function (data) {
    var that = this;
    var orders_data = [];
    if (data.result.focusList.length == size) {
      // console.log('持续加载');
    } else {
      that.setData({
        isloadover: true
    ...
    if (!that.data.isEmpty) {
      // console.log('非第一次加载');
      orders_data = that.data.fans_list.concat(data.result.focusList);//连接多个数组
    } else {
      // console.log('初次加载');
      orders_data = data.result.focusList;
      that.setData({
        isEmpty: false
      });
    }
  ...
  guanzhu: function (e) {
    ...
    if (e.currentTarget.dataset.relationship == 3 || e.currentTarget.dataset.relationship == 2) {
      //取消关注
      url.ajaxPost(false, url.cancelFocus, {
        ...
        if (e.currentTarget.dataset.relationship == 3){
          ...
          fans_list[e.currentTarget.dataset.i].relationship = 0;
        }
      ...
      url.ajaxPost(false, url.focus, {
        ...
        if (e.currentTarget.dataset.relationship == 1){
          fans_list[e.currentTarget.dataset.i].relationship = 3;
        }else{
          fans_list[e.currentTarget.dataset.i].relationship = 2;
        }

...
var count = 0;//加载页
var size = 20;//每页加载条数
var post_list_url = url.xianxiaguanli
...
Page({
  ...
  callbackData: function (data) {
    ...
    data.orders = data.data.result.downlines;
    for(var i=0;i<data.orders.length;i++){
      data.orders[i].registerTime = util.toDate(data.orders[i].registerTime,'YMDhms');
    }
  ...
  dadianhua:function(e){
    ...
    wx.showModal({
        title: '提示',
        content: '确认拨打用户电话:' + e.currentTarget.dataset.tel,
        success: function (res) {
          if (res.confirm) {
            wx.makePhoneCall({
